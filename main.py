...
async def calculate_vwap(bids):
    if not bids:
        return None
    total_usd = sum(
        float(bid["best_bid"]) * float(bid["best_bid_volume"]) for bid in bids
    )
    ...
    while not stop_event.is_set():
        ...
        if (current_time - last_vwap_time) > timedelta(seconds=5):
            if len(recent_bids) >= 5:
                ...
                ask_prices = [float(bid["best_ask"]) for bid in recent_bids[-5:]]
                _, upper_bound, lower_bound = await tools.bollinger_bands(
        ...
        inventory_data.update(
            {
                ...
                "convert_price": best_bid,
                "total_value": total_usd,
            }
        )
        print(f"\nFinal Inventory Value:")
        print(f"USD: {inventory['Fund']['USD']}")
        print(f"{pair}: {inventory[pair]} (Value: {coin_value} @ {best_bid})")
