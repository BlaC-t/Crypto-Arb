...
async def symbol_formating(symbol_format: str, pair: str) -> str:
    ...
    if not (
        symbol_format.count("x") + symbol_format.count("X") == len(first_part)
        and symbol_format.count("y") + symbol_format.count("Y") == len(second_part)
    ):
...
async def bollinger_bands(bid_prices, ask_prices):
    """
    Calculate Bollinger Bands for a given set of bid and ask prices.
    """
